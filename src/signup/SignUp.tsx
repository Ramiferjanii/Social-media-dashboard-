...
export default function ModernSignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const loginResponse = await axios.post('http://localhost:5000/api/signin', {
        email,
        password
      });

      const { token, userId, email: userEmail, username: userUsername, name: userName, avatar } = loginResponse.data;
